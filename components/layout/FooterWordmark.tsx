...
export function FooterWordmark() {
  return (
    <div aria-hidden className="relative mt-12 overflow-hidden">
      <motion.span
        initial={{ y: "22%", opacity: 0 }}
        whileInView={{ y: "0%", opacity: 1 }}
        viewport={{ once: true, margin: "0px 0px -40px 0px" }}
        transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] }}
        className="block select-none whitespace-nowrap text-center font-display text-[clamp(2rem,7.5vw,5.5rem)] font-medium leading-[0.82] tracking-[-0.03em] text-background/15"
      >
        Novus
      </motion.span>
    </div>
